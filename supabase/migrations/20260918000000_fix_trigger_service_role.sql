-- =============================================================================
-- Migration: Fix service_role and claims detection in handle_profile_update_security
-- Reason: PostgREST sets 'request.jwt.claims' as a JSON string, and auth.role() returns 'service_role'.
-- The previous check looked for 'request.jwt.claim.role' (singular, text) which returned NULL,
-- causing valid backend service_role updates to fail with 'Permissão negada para alterar o status da conta.'
-- =============================================================================
...
BEGIN
  ...
  -- Checks current_user, Supabase auth.role(), and PostgREST claims JSON
  BEGIN
  ...
  IF current_user = 'service_role' 
     ...
     OR (v_jwt_claims ->> 'role' = 'service_role')
